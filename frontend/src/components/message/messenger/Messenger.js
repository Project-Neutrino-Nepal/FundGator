...
const Messenger = () => {
  ...
  useEffect(() => {
    const getConversation = async () => {
      const res = await axios.get(
        "http://localhost:5000/conversation/getbyusername/" + user
      );
  ...
  useEffect(() => {
    const getMessages = async () => {
      try {
        const res = await axios.get(
          "http://localhost:5000/message/getbyconversationId/" + currentChat?._id
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/message/add", message);
